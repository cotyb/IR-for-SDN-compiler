...
class fsm:
	...
	def reversed(self):
		'''
			...
			def gandup(current,symbol):
			next = frozenset([
				prev
				for prev in self.map
				for state in current
				if symbol in self.map[prev] and self.map[prev][symbol][0] == state
			])
			gandup={}
			guard=""
			update=""
			...
			for i in range(len(self.states)):
				if i in next:
					if guard=="":
						guard=self.map[i][symbol][1]
					elif self.map[i][symbol][1]  in guard and self.map[i][symbol][2]  in update:
						guard=guard
					else:
						guard=guard+" or "+self.map[i][symbol][1]
				
					if update=="":
						update=self.map[i][symbol][2]
					elif self.map[i][symbol][2]  in update and self.map[i][symbol][1]  in guard:
						update=update
					else:
						update=update+" or "+self.map[i][symbol][2]
...
def parallel(fsms, test):
	...
	def gandup(current,symbol):                   #计算guard与update
		...
		guard={}
		update={}	
		if	1 in current.keys() and fsms[1].map[current[1]]!="" and 0 in current.keys() and fsms[0].map[current[0]]!="":
			...
			for i in range(len(fsms)):
				...
				and symbol in fsms[i].map[current[i]]:		
					...
					while k<a[0]*a[1]:
						for j in range(a[i]):
							if guard[k+j]=="":
								guard[k+j]=fsms[i].map[current[i]][symbol].values()[0][m]
							else:
								guard[k+j] = guard[k+j]+" and "+fsms[i].map[current[i]][symbol].values()[0][m]
							if update[k+j]=="":
								update[k+j]=fsms[i].map[current[i]][symbol].values()[0][m+1]
							else:
								update[k+j]= update[k+j]+" and "+fsms[i].map[current[i]][symbol].values()[0][m+1]
						m+=2
						k=k+a[i]
		#				print guard,"update",update
			i=0
			j=0
			while i <2*a[0]*a[1]:
				...
				i+=2					
		#	print gandup
		elif 1 not in current.keys() or fsms[1].map[current[1]]=="":
	...
	while i < len(states):
		...
		for symbol in sorted(alphabet, key=key):	#计算map根据之前定义的reversed（）中follow函数得到的result进行计算
			...
			for k in range(len(change1)):
				...
				if map[k][symbol][j][0]==change1[k][0] and map[k][symbol][j][1]==change1[k][1]:
					map[k][symbol][j]=change1[k]
				#print"66666666666" ,len(change1),
				#print "states",states     #这几个print只是用来调试看是否正确的
				#print "finals",finals
...
if __name__ == "__main__":
	a = fsm(
			...
			finals   = {2},
			map      = {
				0    : {"a" : {1:["g1","u1","g9","u9","g10","u10"]}   , "b" : {2:["g2","u2"]}},
	...
	b = fsm(
			...
			finals   = {2},
			map      = {
				0    : {"a" : {2:["g5","u5","g11","u11"]}, "b" : {1:["g6","u6"]}   },
				1    : {"a" : {1:["g7","u7"]}, "b" :{2:["g8","u8"]}},
				2    : "",
