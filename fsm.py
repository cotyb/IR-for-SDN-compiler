...
class fsm:
	...
	def reversed(self):
		...
		return redu(alphabet, initial, final, follow)
...
def parallel(fsms, test):
	...
	def gandup(current,symbol):                   #计算guard与update
		...
		for i in range(len(fsms)):
			...
			and symbol in fsms[i].map[current[i]]:
				if guard=="":
					guard=fsms[i].map[current[i]][symbol].values()[0][0]
				else:
					guard = guard+" and "+fsms[i].map[current[i]][symbol].values()[0][0]
				if update=="":
					update=fsms[i].map[current[i]][symbol].values()[0][1]
				else:
					update= update+" and "+fsms[i].map[current[i]][symbol].values()[0][1]
		gandup[0]=guard
		gandup[1]=update
		return gandup		
	...
	while i < len(states):
		...
		for symbol in sorted(alphabet, key=key):
			try:
				...
				guard= gandup(state,symbol)[0]
				update=	gandup(state,symbol)[1]
				try:
					j = states.index(next)
				except ValueError:
					j = len(states)
...
def redu(alphabet, initial, final, follow):         #reversed函数中计算map
	states = [initial]
	finals = set()
	map = {}
